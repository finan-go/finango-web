...
export default function InstitutionListPage({ params }: { params: { persona: Persona['id']; entityType: EntityType['id'] } }) {
  ...
  return (
    <>
      <div className="container mx-auto max-w-5xl py-12 px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {institutionList.map((inst) => (
            <Card key={inst.id} className="transition-transform duration-300 hover:-translate-y-1 flex flex-col">
              ...
              <div className="p-6 pt-0">
                  <Button variant="outline" className="w-full" onClick={() => setSelectedInstitution(inst)}>
      ...
      {selectedInstitution && (
        <Dialog open={!!selectedInstitution} onOpenChange={(isOpen) => !isOpen && setSelectedInstitution(null)}>
            <DialogContent>
                <DialogHeader>
                ...
                <div className="py-4">
                    <h3 className="font-semibold mb-2">Tasas de interés referenciales:</h3>
                    <p className="text-sm text-muted-foreground bg-secondary p-3 rounded-md">
                        {selectedInstitution.interestRateRange}
                    </p>
                    <p className="text-xs text-muted-foreground mt-2">
                        * Las tasas son referenciales y pueden variar según evaluación crediticia.
                    </p>
                </div>
                <DialogFooter>
                    <DialogClose asChild>
